...
function readZipPackage(data) {
  ...
  for (const central of centralDirectory) {
    try {
      ...
      const entryData = central.method === 0 ? compressed : inflateRawSync(compressed);
      entries.push({
        path: central.path,
...
function writeZipPackage(entries, options = {}) {
  ...
  for (const entry of prepared) {
    ...
    const compressed = entry.compression === "store" ? entry.data : new Uint8Array(deflateRawSync(entry.data, { level: options.compressionLevel ?? 9 }));
...
//# sourceMappingURL=miku-ms-office-core.mjs.map
